...
/* XMLHttpRequest connect to server */
...
const methods = (options.type || 'GET').toUpperCase();
  ...
  let myData = null;   
  let header = options.header || `application/${options.dataType || 'octet-stream'}`;
...
/* set httpRequest method and url */ 
 ...
 /* set body request data if method is POST and not GET */
 httpRequest.setRequestHeader('Content-Type', header);
 
 httpRequest.onload = () => {
    if (httpRequest.status >= 200 && httpRequest.status < 300) {
      const response = options.parse ? JSON.parse(httpRequest.responseText) : httpRequest.responseText;
