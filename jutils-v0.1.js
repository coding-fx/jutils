...
loader(duration = 2000, options) {
  ...
  this.element.style.border = `${settings.border || 5}px solid ${settings.background}`;
...
const myrs45 = new URL(window.location.href);
...
switch(flag) {
 ...
 throw new RangeError('origin does not have setter attribute');
  }    
  case 'host': 
  if(url === undefined) {
 ...
 return true;     
  ...
  case 'push_hash':  
    myrs45.hash = url;
    window.history.pushState({}, '', myrs45.href);   
    return true;    
  case 'push_path':  
    myrs45.pathname = url;
    window.history.pushState({}, '', myrs45.href); 
  ...
  myrs45.search = url;
    ...
    return true;       
  default: 
  throw new TypeError(`${flag} is not a valid selector`);
}    
}
